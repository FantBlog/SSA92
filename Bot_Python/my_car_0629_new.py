...
class DrivingClient(DrivingController):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        ref_mid = (sensing_info.to_middle / (sensing_info.speed+0.001)) * -1.0
        ...
        # 도로 거의 벗어났을때
        if (abs(sensing_info.to_middle) - half_load_width) > 10:
            print("도로 가장자리임 ", sensing_info.to_middle)
            steer_factor -= sensing_info.to_middle * 0.015
        ...
        if full_throttle == False:
            ...
            set_brake = map_value(sensing_info.speed - target_speed, 0, 140, 0, 1)
        ...
        if len(sensing_info.track_forward_obstacles) > 0 and sensing_info.track_forward_obstacles[0]['dist'] <= 90 and sensing_info.speed >= 120:
            ...
            for i in range(len(sensing_info.track_forward_obstacles)):
                ...
                if mid_dis < 2.5:
                    ...
                    # set_brake = map_value(target_dis, 0, 160, 0, 0.8)
                    print("front object", set_brake)
                    # set_throttle = 0
...
## 1. 코너 경사 정도를 알 수 있음
def prepare_corner(road_data, to_mid):
    x, y = road_data # y가 음수면 왼, 양수면 오른, |y|가 클수록 많이 꺾임
    is_in = y*to_mid
    return is_in
...
def path_planning(car_speed, car_yaw, car_pos, forward_map, half_road_width, obstacles) -> int:
    ...
    if 0<= car_position < num_cells:
        ...
        for idx, point in enumerate(forward_map):
            ...
            for i in range(1,int(1/grid_size * 1.51)):  # 차량의 폭을 고려, 그리드가 0.1m 이므로 1m씩 추가 및 여유 0.5m 추가

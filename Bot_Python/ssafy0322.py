...
from math import asin, atan2, tan, pi, cos, sin, ceil, sqrt
...
class DrivingClient(DrivingController):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        ## 0. 기본값 세팅
        angle_num = max(int(sensing_info.speed / 45), 1)
        ...
        if not self.is_collided and (abs(ref_angle) >= 4 or abs(sensing_info.moving_angle) >= 10):# prepare_corner(reli_routeInform)):
        ...
        # 도로 거의 벗어났을때
        if (abs(sensing_info.to_middle) - half_load_width) > 5:
            print("도로 가장자리임 ", sensing_info.to_middle)
            steer_factor -= sensing_info.to_middle * 0.02
        ...
        road_range = int(sensing_info.speed / 17)
        max_angle = 0
        for i in range(0, road_range):
            ...
            # 결승선
            if abs(sensing_info.track_forward_angles[i] - sensing_info.track_forward_angles[i-1]) > 140:
                full_throttle = True
                emergency_brake = False
        ...
        if len(sensing_info.track_forward_obstacles) > 0 and sensing_info.track_forward_obstacles[0]['dist'] <= 90 and sensing_info.speed >= 120:
            ...
            for i in range(len(sensing_info.track_forward_obstacles)):
                ...
                if mid_dis < 2.5:
                    ...
                    set_brake = map_value(target_dis, 0, 160, 0, 1) * 0.6
                    print("전방 장애물 set_brake", set_brake)
                    # set_throttle = 0

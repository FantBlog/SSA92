...
class DrivingClient(DrivingController):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        if not self.is_collided and (abs(ref_angle) >= 10 or abs(sensing_info.moving_forward) >= 10):# prepare_corner(reli_routeInform)):
        ...
        if len(sensing_info.track_forward_obstacles) == 0 or abs(sensing_info.to_middle) > 8:
        ...
        for i in range(0, road_range):
            ...
            # print("angle", fwd_angle)
            max_angle = max(max_angle, fwd_angle)
            if fwd_angle > 40 and sensing_info.speed > 50: 
                full_throttle = False
            if fwd_angle > 80  and sensing_info.speed > 50:  ## 커브가 80도 이상인 경우 steering 까지 추가로 제어
                emergency_brake = True
                break
            # 결승선
            if abs(sensing_info.track_forward_angles[i] - sensing_info.track_forward_angles[i-1]) >= 160:
                full_throttle = True
                emergency_brake = False
                # print("goal line", abs(sensing_info.track_forward_angles[i] - sensing_info.track_forward_angles[i-1]))
                break
        ...
        if full_throttle == False:
            ...
            set_brake = map_value(sensing_info.speed - target_speed, 0, 160, 0, 1) # checking...
        ...
        if emergency_brake:
            ...
            if set_steering > 0:
                set_steering += 0.13 # 0.1 or 0.13
            else:
                set_steering -= 0.13
            set_brake = 0.7
        
        if sensing_info.speed > 190:
            set_brake = 0.8
            set_throttle = -0.2
        ...
        if len(sensing_info.track_forward_obstacles) > 0 and sensing_info.track_forward_obstacles[0]['dist'] <= 90 and sensing_info.speed >= 120:
            ...
            for i in range(len(sensing_info.track_forward_obstacles)):
                ...
                if mid_dis < 2.5:
                    # print("감속", mid_dis)
                    target_dis = map_value(sensing_info.track_forward_obstacles[0]['dist'], 0, 90, 180, 50)
                    set_brake = map_value(target_dis, 0, 160, 0, 1) * 0.6 # checking...
                    # print(set_brake)
                    set_throttle = 0.2
...
def path_planning(car_speed, car_yaw, car_pos, forward_map, half_road_width, obstacles) -> int:
    ...
    if 0<= car_position < num_cells:
        ...
        for idx, point in enumerate(forward_map):
            ...
            score_closest_angle = calculate_weight(car_yaw, target_angle, 50, max_score=48)
            ...
            score_closest_point = calculate_weight(car_position, idx, num_cells, max_score=52) 
